...
import complaintColumns from '../json/complaint-columns.json'
...
export default {
  ...
  data () {
    return {
      urlObj: {
        complaintlist: this.$api.state.Custom.complaintlist.url,
        getrepairtype: this.$api.state.Custom.getrepairtype.url,
        searchrooms: this.$api.state.Custom.searchrooms.url,
        addcomplaint: this.$api.state.Custom.addcomplaint.url,
        complaintdetail: this.$api.state.Custom.complaintdetail.url,
        complaintUser: this.$api.state.Custom.complaintUser.url,
        compdispatch: this.$api.state.Custom.compdispatch.url,
        nmentusers: this.$api.state.Custom.nmentusers.url,
        complaintchange: this.$api.state.Custom.complaintchange.url,
        complreturn: this.$api.state.Custom.complreturn.url,
        complainthandle: this.$api.state.Custom.complainthandle.url,
        complaintclose: this.$api.state.Custom.complaintclose.url,
  ...
  methods: {
    ...
    getTypeList () {
      this.$axios
        .post(this.urlObj.getrepairtype, { type: 2 })
        .then(res => {
          if (res.Code === 200) {
            this.typeOptions = res.Data ? res.Data : []
          } else {
            let msg = res.Message ? res.Message : '获取投诉类型失败！'
            this.$message({
              type: 'error',
              message: msg
            })
    ...
    tableLoad () {
      ...
      // 获取项目列表数据
      this.$axios
        .post(this.urlObj.complaintlist, data)
        .then(res => {
          if (res.Code === 200) {
    ...
    formSubmit () {
      this.$refs.ruleForm.validate(valid => {
        if (valid) {
          ...
          this.$axios
            .post(this.urlObj.addcomplaint, data)
            .then(res => {
              if (res.Code === 200) {
                this.$message({
                  message: '新增投诉建议成功！',
                ...
                let msg = res.Message ? res.Message : '新增投诉建议失败！'
                this.$message({
                  message: msg,
                  type: 'error'
                })
    ...
    getDetailData (id) {
      this.isLoading = true
      this.$axios
        .post(this.urlObj.complaintdetail, { id: id })
        .then(res => {
          if (res.Code === 200) {
            res.Data.hasRecord =
            ...
            let msg = res.Message ? res.Message : '获取投诉详情数据失败！'
            this.$message({
              type: 'error',
              message: msg
            })
    ...
    // 点击闭单按钮处理
    orderClose (index) {
      this.$confirm('此操作将关闭当前投诉建议订单，确定继续操作吗?', '提示', {
        ...
        .then(() => {
          ...
          this.$axios
            .post(this.urlObj.complaintclose, data)
            .then(res => {
              if (res.Code === 200) {
                this.$message({
                  message: '闭单成功！',
    ...
    getHandleUser () {
      this.$axios
        .post(this.urlObj.complaintUser, { id: this.detailObj.id })
        .then(res => {
          if (res.Code === 200) {
    ...
    sendOrders () {
      if (!this.userVal && !this.content) {
        ...
        this.$axios
          .post(this.urlObj.compdispatch, data)
          .then(res => {
            if (res.Code === 200) {
              this.$message({
                type: 'success',
    ...
    reassignOrder () {
      if (!this.reassign) {
        ...
        this.$axios
          .post(this.urlObj.complaintchange, data)
          .then(res => {
            if (res.Code === 200) {
              this.$message({
                type: 'success',
    ...
    // 确认回访处理
    returnVisit () {
      if (!this.content && !this.isSatisfa && this.isSatisfa !== 0) {
        this.$message({
          type: 'warning',
          message: '请填写回访结果、是否满意！'
        })
      } else if (!this.content) {
        this.$message({
          type: 'warning',
      ...
      } else if (this.isSatisfa !== 0 && !this.isSatisfa) {
        this.$message({
          type: 'warning',
          message: '请选择是否满意！'
        })
        ...
        let data = {
          id: this.detailObj.id,
          content: this.content,
          is_satisfy: this.isSatisfa
        }
        this.$axios
          .post(this.urlObj.complreturn, data)
          .then(res => {
            if (res.Code === 200) {
              this.$message({
                type: 'success',
    ...
    handleFinish () {
      if (!this.content) {
        ...
        this.$axios
          .post(this.urlObj.complainthandle, data)
          .then(res => {
            if (res.Code === 200) {
              this.$message({
                type: 'success',
        ...
        this.$axios
          ...
          .then(res => {
            if (res.Code === 200) {
              ...
              let msg = res.Message ? res.Message : '投诉建议数据获取失败！'
              this.$message({
                type: 'error',
                message: msg
              })
