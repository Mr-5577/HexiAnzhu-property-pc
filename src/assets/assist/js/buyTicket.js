import dapiaoCloumns from '../json/dapiao-cloumns.json'
import shoukuanCloumns from '../json/shoukuan-cloumns.json'
...
export default {
  ...
  data () {
    return {
      urlObj: {
        ...
        batchopensn: this.$api.state.Assist.batchopensn.url,
        nopayorder: this.$api.state.Assist.nopayorder.url,
        batchcollectionsn: this.$api.state.Assist.batchcollectionsn.url,
  ...
  computed: {
    // 表格列数据配置
    columns () {
      return this.typeVal == 1 ? dapiaoCloumns.list : shoukuanCloumns.list
    }
  },
  ...
  methods: {
    ...
    tableLoad () {
      let url =
        this.typeVal == 1 ? this.urlObj.subjectarrears : this.urlObj.nopayorder
      // 表格处于加载状态
        ...
        this.$confirm('确定要打印当前票据吗?', '提示', {
          ...
          .then(() => {
            let data = {
              ...
              data: [
                {
                  resources_id: this.tableData[index].resourcesmodel_id,
                  resources_type: this.tableData[index].resourcesmodel_type,
                  oid: this.tableData[index].oid,
                  subject_id: this.tableData[index].subject_village_id,
                  time: this.tableData[index].time,
                  use_info: this.tableData[index].use_info,
                  resources_name: this.tableData[index].resources_name
                }
              ]
            }
    ...
    printCommit (data) {
      this.$axios
        .post(this.urlObj.batchopensn, data)
        .then(res => {
          if (res.Code === 200) {
            this.$message({
              message: '打印票据成功！',
            ...
            let msg = res.Message ? res.Message : '打印票据失败！'
            this.$message({
              message: msg,
              type: 'error'
            })
    ...
    confirm () {
      if (this.radioVal && this.dateVal) {
        ...
        let arr = []
        if (this.isBatch) {
          arr = this.tableSelected.map(item => item.id)
        } else {
          arr = [this.tableData[this.currentIndex].id]
        }
        let data = {
        ...
        this.$axios
          .post(this.urlObj.batchcollectionsn, data)
          .then(res => {
            if (res.Code === 200) {
              this.$message({
                message: '确认收款成功！',
